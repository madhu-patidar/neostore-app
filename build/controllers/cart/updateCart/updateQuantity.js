...
var updateQuantityByCustId = function (req, res) {
                ...
                cartModel_1.default.find({ customer_id: customer_id, product_id: product_id }, function (err, result) {
                    if (err)
                        ...
                        if (req.body.quantity) {
                            cartModel_1.default.updateOne({ customer_id: customer_id, product_id: product_id }, { quantity: req.body.quantity }, { new: true }, function (err, result) {
                                if (err)
                                    res.status(404).json({ success: false, error_message: err });
                            });
                        }
...
exports.default = updateQuantityByCustId;
