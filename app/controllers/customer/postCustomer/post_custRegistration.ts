...
const insertCustomerData=(req:Request,res:Response)=>{
   ...
   Joi.validate({u_email:req.body.email,u_pass:req.body.pass, u_cofirmPass:req.body.confirmPass,u_MobNumber:req.body.phone_no,u_gender:req.body.gender},schema,(err,result)=>{
    if (err) {
        ...
        if(pass.localeCompare(confirm_pass)==0){
            ...
            res.status(404).json({success:"false",message:"Password and confirm passwod should be same"})
        }
    }
