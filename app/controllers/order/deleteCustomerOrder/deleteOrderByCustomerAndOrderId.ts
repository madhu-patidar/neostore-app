...
//Delete All Orders
const deleteOrderByCustomerAndOrderId=(req:Request,res:Response)=>{
  let customer_id=parseInt(req.body.id)
  let order_id=req.body.order_id
    OrderModel.deleteMany({customer_id:customer_id,order_id:order_id})
    .then(result=>{
        if(result.n!==0)
...
export default deleteOrderByCustomerAndOrderId